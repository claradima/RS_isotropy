...
///    g++ -g -std=c++1y GetPMTPositions400Missing.cc -o GetPMTPositions400Missing `root-config --cflags --libs` -I${RATROOT}/include/libpq -I${RATROOT}/include -I${RATROOT}/include/external -L${RATROOT}/lib -lRATEvent_Linux
/// You can access other locations in rat, but need special command
///
/// To run code after compiling, use command: ./GetPMTPositions400Missing
///
...
void GetPMTPosDir(const RAT::DU::PMTInfo& pmtinfo) {
    ...
    std::cout << NPMTS << " PMTs found" << std::endl;

    // Open a file for writing
    std::ofstream outputFile("pmt_positions_400_missing.csv");

    // Loop through the PMTs and compute position and direction
    for (int it = 400; it < NPMTS; it++) {
