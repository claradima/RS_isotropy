...
print("Click on points, press Enter to finish.")
clicked_points = []
while True:
    click = plt.ginput(n=1, timeout=0)
    if not click:
        break
    clicked_points.extend(click)
    # Plot the clicked point
    plt.scatter(click[0][0], click[0][1], s=30, color='red')
...
clicked_points_df.to_csv('More_Nodes_Grid.csv', index=False)
